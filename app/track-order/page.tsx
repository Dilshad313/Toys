...
// Demo orders (shown if no real orders)
const demoOrders: Order[] = [
  {
    id: 'demo-1',
    orderNumber: '#ATHVI-1001',
    createdAt: '2026-08-05T10:30:00Z',
    totalPrice: { amount: '2497.00', currencyCode: 'INR' },
    fulfillmentStatus: 'FULFILLED',
    financialStatus: 'PAID',
    lineItems: {
      edges: [
        { node: { title: 'Elephant Drummer Toy', quantity: 1, variant: { price: { amount: '1299.00' } } } },
        { node: { title: 'RC Stunt Car', quantity: 1, variant: { price: { amount: '899.00' } } } },
        { node: { title: 'Activity Cube', quantity: 1, variant: { price: { amount: '299.00' } } } }
      ]
    },
    shippingAddress: {
      firstName: 'John',
  ...
  },
  {
    ...
    lineItems: {
      edges: [
        { node: { title: 'Soft Teddy Bear', quantity: 1, variant: { price: { amount: '1299.00' } } } }
      ]
    },
    shippingAddress: {
      firstName: 'Jane',
  ...
  {
    id: 'demo-3',
    orderNumber: '#ATHVI-1003',
    createdAt: '2026-07-28T09:15:00Z',
    totalPrice: { amount: '899.00', currencyCode: 'INR' },
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        if (result.success && result.data?.orders?.edges && result.data.orders.edges.length > 0) {
          ...
          setUsingDemo(true)
          setError(result.message || 'No orders found. Showing demo orders.')
        }
      } catch (error) {
        ...
        setOrders(demoOrders)
        setUsingDemo(true)
        setError('Unable to fetch orders. Showing demo orders.')
      } finally {
        setLoading(false)
      }
    }
    ...
    <div className="bg-gray-50 min-h-screen py-12 md:py-20">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <div className="w-20 h-1 bg-[#D32F2F] mx-auto mt-4 rounded-full" />
          {usingDemo && (
            <p className="text-xs text-yellow-600 mt-2 bg-yellow-50 inline-block px-3 py-1 rounded-full">
              ⚡ Showing demo orders
            </p>
          )}
        </motion.div>
